...
function SettingRow({ label, description, checked, onChange, color = "primary", science }: {
  ...
  return (
    <div className="flex items-start justify-between gap-6 py-4 border-b border-border/40 last:border-b-0 group">
...
function FolderSetting({ label, description, value, placeholder, onPick, onClear, disabled }: {
  ...
  return (
    <section className="space-y-3">
      ...
      {value ? (
        <div className="rounded-xl border border-border/50 bg-muted/20 px-4 py-3.5 space-y-3">
          <div className="flex items-start gap-2.5">
            <IconFolderCheck size={16} className="mt-0.5 shrink-0 text-green-500" />
            <span className="break-all text-sm text-foreground font-medium leading-relaxed" title={value}>{value}</span>
          </div>
          <div className="flex gap-2 pt-1">
            <Button variant="outline" size="sm" className="h-8 gap-2 text-xs" onClick={onPick} disabled={disabled}>
              <IconFolderOpen size={14} />Cambiar…
            </Button>
        ...
        <div className="rounded-xl border border-dashed border-border/60 p-4 space-y-3 bg-muted/5">
          <div className="flex items-center gap-2 text-muted-foreground">
            <IconAlertCircle size={16} className="shrink-0 text-amber-500" />
            <span className="text-sm font-medium">{placeholder}</span>
          </div>
          <Button variant="outline" size="sm" className="h-8 gap-2 text-xs" onClick={onPick} disabled={disabled}>
            <IconFolderOpen size={14} />Seleccionar carpeta…
          </Button>
        </div>
      )}
    </section>
...
function SectionTitle({ children, color }: { children: React.ReactNode; color?: string }) {
  return (
    <div className="mb-8 flex items-center gap-3">
      {color && <div className={cn("h-6 w-1.5 rounded-full shadow-sm", color)} />}
      ...
      <div className="flex gap-4">
        {THEMES.map(({ value, icon: Icon, label, preview }) => (
          <button key={value} onClick={() => setTheme(value)}
            className={cn(
              "flex flex-1 flex-col items-center gap-3 rounded-xl border-2 p-5 transition-all duration-200",
              theme === value
    ...
    <div>
      ...
      <div className="py-4">
        <div className="flex items-center gap-2 mb-4">
          <p className="text-sm font-semibold text-foreground">Ambiente Sonoro</p>
...
export function SettingsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="relative flex w-[920px] h-[660px] max-w-[95vw] max-h-[90vh] rounded-2xl bg-background border border-border shadow-[0_8px_32px_rgba(0,0,0,0.2)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.6),0_0_0_1px_rgba(255,255,255,0.05)] overflow-hidden animate-in zoom-in-95 fade-in duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Left sidebar */}
        <div className="w-[240px] shrink-0 flex flex-col border-r border-border bg-sidebar/50">
          ...
          <nav className="flex-1 overflow-y-auto px-3 pb-6 space-y-6">
            {groups.map((group) => (
              <div key={group}>
                <p className="px-3 mb-2 text-[11px] font-bold uppercase tracking-widest text-muted-foreground/50">{group}</p>
                <div className="space-y-0.5">
                  {CATEGORIES.filter((c) => c.group === group).map(({ id, label, icon: Icon, color }) => (
                    ...
                    >
                      <Icon size={16} strokeWidth={2} className={cn("transition-colors", activeCategory === id ? color : "text-muted-foreground/60 group-hover:text-muted-foreground")} />
